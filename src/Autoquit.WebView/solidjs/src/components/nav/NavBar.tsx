...
export default function NavBar(props: JSX.HTMLAttributes<HTMLButtonElement>){
    ...
    return <div class="flex space-x-3">
        <div>
            <button class={`rounded-full outline-none w-12 h-12 text-${state.getTextColour?.call(null, state)} bg-${state.getTextColour?.call(null, state)} bg-opacity-0 hover:bg-opacity-5`} onClick={props.onClick}>
